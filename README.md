...
- **Main Event Loop**: Handles Discord commands, queue management, and user interactions
- **Queue Worker**: Async background process that processes requests sequentially
- **AI Model**: Runs in separate threads (managed by the AI library) to prevent blocking
...
### **Architecture:**
- **Main Event Loop**: Single async thread handling Discord commands and queue management
- **Queue Worker**: Async coroutine that processes requests one by one
- **AI Generation**: Separate threads (managed by PyTorch/Stable Diffusion) for GPU/CPU work
- **Non-blocking**: Bot stays responsive during generation thanks to async programming
